...
export default class Reminders extends React.Component {
  ...
  getData = (id) => {
    if(this.state.token){
      ...
      try{
        fetch(`${BasePath}recent_reminders.php`, {
          method: 'POST',
          headers: {
    ...
    if(data.status == "Completed"){
      return(
        <View style={{flexDirection: 'row',}}>
          ...
          <Text style={{textDecorationLine: 'underline',fontSize:WP('4'),color:'red',textAlign: 'center'}} onPress={() => this.props.navigation.navigate('Preview',{item_id: data.id})}>Complete </Text>
        </View>
      )
    }else{
      return(
        <View style={{flexDirection: 'row',}}>
          <Text style={{fontSize:WP('4'),color:'red',textAlign: 'center'}}>Status: </Text>
          <Text style={{textDecorationLine: 'underline',fontSize:WP('4'),color:'red',textAlign: 'center'}} onPress={() => this.props.navigation.navigate('Upload',{item_id: data.id})}>InComplete</Text>
        </View>
      )
