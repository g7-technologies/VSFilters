...
import { ScrollView ,StatusBar,ImageBackground,AsyncStorage,Image,ActivityIndicator,TouchableOpacity,Dimensions,StyleSheet, Text, View, FlatList } from 'react-native';
...
export default class UnCompleted extends React.Component {
  ...
  getData = () => {
    if(this.state.token){
      ...
      try{
        fetch(`${BasePath}manager_incompleted_task.php`, {
          method: 'POST',
          headers: {
    ...
    if(this.state.initialArr.length > 0){
      return this.state.initialArr.map((item) => {
        return(
          ...
          <Text style={{fontSize:WP('4'),color: 'green'}}>Address: {item.property}</Text>
