...
export default class Completed extends React.Component {
  ...
  getData = () => {
    if(this.state.token){
      ...
      try{
        fetch(`${BasePath}manager_completed_task.php`, {
          method: 'POST',
          headers: {
  ...
  render_reminders = () =>{
    if(this.state.isloading && this.state.initialArr){
      return this.state.initialArr.map((item) => {
        return(
          <TouchableOpacity onPress={() => this.props.navigation.navigate('ManagerPreview',{item_id: item})}>
        ...
        <View style={styles.container}>
          ...
          <View style={{flex:2,width: windowWidth,flexDirection: 'row', backgroundColor:'black'}}>
            <View style= {{flex:0.5,justifyContent: 'center',alignItems: 'flex-start'}} >
              <TouchableOpacity onPress={() => this.props.navigation.goBack('ManagerHome')}>
