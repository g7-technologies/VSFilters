...
import { Alert,Picker,ScrollView,StatusBar,ImageBackground,AsyncStorage,Image,ActivityIndicator,TouchableOpacity,Dimensions,StyleSheet, Text, View,
  ImageStore,
  ImageEditor,
  Platform,
} from 'react-native';
...
export default class UploadImage extends React.Component {
  ...
  save_data = () => {
    ...
    if(this.state.base64_str ){
      ...
      formData.append("manager_id",this.state.token);
      ...
      try{
        fetch(`${BasePath}manager_save_reminder.php`, {
          method: 'POST',
          headers: {
        ...
        .then((responseJson) => {
          if(responseJson.error == false){
            ...
            Alert.alert(
              ...
              [
                {text: 'Cancel', onPress: () => this.props.navigation.navigate('ManagerHome'), style: 'cancel'},
                {text: 'OK', onPress: () => this.props.navigation.navigate('ManagerHome')},
              ],
              { cancelable: false }
            )
        ...
        <View style={styles.container}>
          ...
          <View style={{flex:2,width: windowWidth,flexDirection: 'row', backgroundColor:'black'}}>
            <View style= {{flex:0.7,justifyContent: 'center',alignItems: 'flex-start'}} >
              <TouchableOpacity onPress={() => this.props.navigation.navigate('ManagerReminder')}>
                <Image source={require('../assets/left.png')} style={{marginLeft: 10,height:25,width:25}} />
              </TouchableOpacity>
            </View>
            <View style={{flex:1.3,justifyContent: 'center',alignItems: 'flex-start',paddingRight:10}}>
          ...
          <View style={{flex:10}}>
            ...
            <View style={{flex:10}}>
              ...
              {Platform.OS=='ios'?
              <View style={{flex:3,width:'80%',alignSelf:'center',justifyContent:'center'}}>
                  <Picker
                      style={{fontSize:10,color:'#fff'}}
                      selectedValue={this.state.pickerSelection}
                      onValueChange={(itemValue,itemIndex) => this.setState({pickerSelection: itemValue})}>
                      <Picker.Item label="1 month" value="1" key="1"/>
                      <Picker.Item label="2 months" value="2" key="2"/>
                      <Picker.Item label="3 months" value="3" key="3"/>
                      <Picker.Item label="4 months" value="4" key="4"/>
                      <Picker.Item label="6 months" value="6" key="5"/>
                    </Picker>
                 <View style={{flex:1}}/>
              </View>
              :
